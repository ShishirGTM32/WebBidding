...
class HomeView(ListView):
    ...
    def get_queryset(self):
        from .signals import check_and_notify_auction_end
        check_and_notify_auction_end()
...
class PlaceBidView(LoginRequiredMixin, View):
    def post(self, request, bid_id):
        ...
        # Save the bid
        new_bid_item = BidItem.objects.create(
            bid=bid, 
            bidder=request.user, 
        ...
        previous_highest = BidItem.objects.filter(bid=bid).exclude(
            id=new_bid_item.id
        ...
        if previous_highest:
            Notification.objects.create(
                ...
                message=f"You've been outbid on '{bid.item_name}'. Current highest bid: ${bid_amount}",
                is_read=False
            )
        ...
        Notification.objects.create(
            user=request.user,
            message=f"Your bid of ${bid_amount} for '{bid.item_name}' has been placed successfully!",
            is_read=False
        )
...
class SellerDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['ended_bids'] = Bid.objects.filter(
            user=self.request.user,
...
class BidderDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for bid_item in bid_items:
            ...
            # Get the highest bid for this specific auction
            highest_bid_item = BidItem.objects.filter(
                bid=bid_item.bid
            ).order_by('-bid_amount').first()

            # Check if current user's bid is the highest
            is_highest_bid = (highest_bid_item and 
                            highest_bid_item.bidder == self.request.user and 
                            highest_bid_item.id == bid_item.id)

            bids_placed.append({
                'bid': bid_item.bid,
                'item': bid_item.bid.item_name,
